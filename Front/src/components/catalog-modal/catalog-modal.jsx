...
const CatalogModal = ({ catKey, id, setShow }) => {
    ...
    if (id) {
      const getCatalog = async () => {
        const res = await axios.get(
          `${process.env.REACT_APP_SERVER_API}/api/subcatalog/only/${id}`
        );
  ...
  const handleSubmite = async (e) => {
    ...
    if (!id) {
      ...
      await axios
        .put(
          `${process.env.REACT_APP_SERVER_API}/api/subcatalog/${id}`,
          formData
        )
        .then((res) => {
          if (res.status === 201) window.location.reload();
    ...
    <>
      <div className="catalog-modal">
        <form onSubmit={handleSubmite} className="catalog-block">
          ...
          />
          <button type="submit">{id ? "Змінити" : "Створити"}</button>
        </form>
      </div>
    </>
  );
